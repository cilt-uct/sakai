...
const gradableDataMixin = Base => class extends Base {
  loadGradableData(gradableId, courseId, submissionId) {
    ...
    return new Promise(resolve => {
      fetch(`/direct/assignment/gradable.json?gradableId=${gradableId}`, {
        cache: "no-cache",
        credentials: "same-origin"
      }).then(res => res.json()).then(gradableData => {
        ...
        this.groups = gradableData.groups; // Load up the graded status for each submission. We need that for the next/prev ungraded type of navigations.

        fetch(`/direct/assignment/grades.json?gradableId=${gradableId}&courseId=${courseId}`, {
          cache: "no-cache",
          credentials: "same-origin"
        }).then(res => res.json()).then(gradesData => {
