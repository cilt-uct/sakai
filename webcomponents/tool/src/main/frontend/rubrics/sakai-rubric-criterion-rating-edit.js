...
import { ifDefined } from "/webcomponents/assets/lit-html/directives/if-defined.js?version=9c647cc0";
...
export class SakaiRubricCriterionRatingEdit extends RubricsElement {
  ...
  saveEdit(e) {
    ...
    this.rating.points = parseFloat(document.getElementById(`rating-points-${this.rating.id}`).value); // Enforce a points value. Blank breaks things.

    if (isNaN(this.rating.points)) this.rating.points = 0.0; // Round user input to two digits
    ...
    this.resetFields();
    this.dispatchEvent(new CustomEvent('save-rating', {
      detail: this.rating
    }));
    this.hideToolTip();
  }
  ...
  deleteRating(e) {
    e.stopPropagation();
    this.rating.criterionId = this.criterionId;
    this.dispatchEvent(new CustomEvent('delete-rating', {
      detail: this.rating
    }));
    this.hideToolTip();
  }
