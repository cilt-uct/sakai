...
import "./sakai-rubric-grading-comment.js?version=9c647cc0";
import { SakaiRubricsLanguage, tr } from "./sakai-rubrics-language.js?version=9c647cc0";
export class SakaiRubricGrading extends RubricsElement {
  ...
  toggleRating(e) {
    ...
    const ratingId = parseInt(e.currentTarget.dataset.ratingId); // Look up the criterion and rating objects
    ...
    if (rating.selected) {
      ...
      const auxPoints = this.rubric.weighted ? (rating.points * (criterion.weight / 100)).toFixed(2) : rating.points;
