...
import "./sakai-rubric-readonly.js?version=9c647cc0";
import { SakaiRubricsHelpers } from "./sakai-rubrics-helpers.js?version=9c647cc0";
...
export class SakaiRubricsSharedList extends RubricsElement {
  ...
  getSharedRubrics(token) {
    ...
    SakaiRubricsHelpers.get("/rubrics-service/rest/rubrics/search/shared-only", token, {
      params
  ...
  copyToSite(e) {
    var options = {
      extraHeaders: {
        "x-copy-source": e.detail,
        "lang": this.locale
      }
    };
    SakaiRubricsHelpers.post("/rubrics-service/rest/rubrics/", this.token, options).then(() => this.dispatchEvent(new CustomEvent("copy-share-site")));
  }
