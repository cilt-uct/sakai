...
import { repeat } from "/webcomponents/assets/lit-html/directives/repeat.js?version=9c647cc0";
import "./sakai-rubric-edit.js?version=9c647cc0";
import "./sakai-item-delete.js?version=9c647cc0";
import "./sakai-rubric-criterion-edit.js?version=9c647cc0";
import "./sakai-rubric-criterion-rating-edit.js?version=9c647cc0";
import { SharingChangeEvent } from "./sharing-change-event.js?version=9c647cc0";
import { Sortable } from "/webcomponents/assets/sortablejs/modular/sortable.esm.js?version=9c647cc0";
...
export class SakaiRubricCriteria extends RubricsElement {
  ...
  handleSortedCriteria() {
    ...
    $.ajax({
      ...
      data: urlList
    }).done(() => this.letShareKnow()).fail((jqXHR, error, message) => {
      console.error(error);
  ...
  handleSortedCriterionRatings(e) {
    ...
    const sortedIds = Array.from(this.querySelectorAll(`#cr-table-${criterionId} .rating-item`)).map(r => r.dataset.ratingId); // Reorder the ratings based on the sort result
  ...
  deleteCriterion(e) {
    ...
    this.requestUpdate();
    this.letShareKnow();
    this.dispatchEvent(new CustomEvent('refresh-total-weight', {
      detail: {
  ...
  addRating(e) {
    ...
    $.ajax({
      ...
      method: "POST",
      data: "{}"
    }).done(data => this.addRatingResponse(criterionId, ratingPos, data)).fail((jqXHR, error, message) => {
      console.error(error);
      console.info(message);
    });
  }
  ...
  deleteRating(e) {
    ...
    const urlList = criterion.ratings.slice().reverse().reduce((a, v) => {
      return `${baseUrl}${v.id}\n${a}`;
    }, '');
  ...
  cloneCriterion(e) {
    ...
    $.ajax({
      ...
      method: "POST",
      data: "{}"
    }).done(data => this.createCriterionResponse(data)).fail((jqXHR, error, message) => {
      console.error(error);
      console.info(message);
    });
  }
  ...
  createCriterionResponse(nc) {
    ...
    this.criteriaMap.set(nc.id, nc); // Add the criterion to the rubric
