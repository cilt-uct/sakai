...
export class SakaiRubricCriterionEdit extends RubricsElement {
  ...
  editCriterion(e) {
    ...
    if (!this.classList.contains("show-tooltip")) {
      ...
      popover.show(); // and highlight the title
  ...
  hideToolTip() {
    ...
    this.classList.remove("show-tooltip"); // fire hide-tooltip event to allow parent components to take action.
  ...
  cancelEdit(e) {
    e.stopPropagation(); // revert changed data

    this.criterionClone.title = this.criterion.title;
    this.criterionClone.description = this.criterion.description; // hide popover
  ...
  saveEdit(e) {
    ...
    }).done(data => this.updateUi(data)).fail((jqXHR, error, message) => {
      console.log(error);
      console.log(message);
    }); // hide the popover
  ...
  updateUi(data) {
    this.hideToolTip();
    this.dispatchEvent(new CustomEvent('criterion-edited', {
      detail: {
        id: data.id,
        title: data.title,
