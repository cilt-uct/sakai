import { SakaiElement } from "./sakai-element.js?version=9c647cc0";
...
class SakaiEditor extends SakaiElement {
  ...
  attachEditor() {
    ...
    if (sakai?.editor?.launch) {
      ...
      this.editor = CKEDITOR.replace(this.elementId, {
        toolbar: SakaiEditor.toolbars.get("basic")
      });
    }
...
SakaiEditor.toolbars.set("basic", [{
  name: 'document',
  items: ['Source', '-', 'Bold', 'Italic', 'Underline', '-', 'Link', 'Unlink', '-', 'NumberedList', 'BulletedList', 'Blockquote']
}]);